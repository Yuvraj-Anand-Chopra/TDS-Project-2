...
# Initialize LLM (with correct API for langchain-google-genai 1.0.5)
...
def handle_malformed_node(state: AgentState):
    ...
    return {
        "messages": [
            HumanMessage(
                content="SYSTEM ERROR: Your last tool call was malformed (invalid JSON). Please rewrite the code and try again. Ensure you escape newlines and quotes correctly."
            )
        ]
    }
...
def agent_node(state: AgentState):
    ...
    if prev_time is not None:
        ...
        if diff >= 180 or (offset != "0" and (cur_time - float(offset)) > 90):
            ...
            fail_instruction = ("You have exceeded the time limit. Immediately call post_request and submit a WRONG answer.")
